...
public class stdattendance extends AppCompatActivity {
    ...
    private void fetchUserOrders() {
        ...
        Intent intent1 = getIntent();
        String userIdp = intent1.getStringExtra("id");
        String url = "http://10.0.2.2/pavaniapp/stdattendance.php"; // Replace with your PHP script's URL
                    ...
                    if (jsonResponse.getBoolean("status")) {
                        ...
                        if (jsonArray.length() > 0) {
                            for (int i = 0; i < jsonArray.length(); i++) {
                                ...
                                String date = order.getString("courseid");
                                String mobile = order.getString("attendedclasses");
                                String service = order.getString("totalclasses");
                                Integer a=Integer.parseInt(mobile);
                                Integer b=Integer.parseInt(service);
                                Integer c;
                                if(b!=0) {
                                    c = (a * 100) / b;
                                }
                                ...
                                String actions = Integer.toString(c);
                                ...
                                styledDetails.append(getBoldText("Club Name: ")).append(name).append("\n");
                                styledDetails.append(getBoldText("Club Code: ")).append(date).append("\n");
                                styledDetails.append(getBoldText("Attended Classes: ")).append(mobile).append("\n");
                                styledDetails.append(getBoldText("Total Classes: ")).append(service).append("\n");
                                styledDetails.append(getBoldText("Percentage: ")).append(getRedText(actions)).append("%\n\n");
